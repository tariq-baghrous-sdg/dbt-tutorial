locals {
  spec       = yamldecode(file(var.product_spec))
...
resource "google_storage_bucket" "bundle_bucket" {
  ...
  dynamic "encryption" {
    for_each = try(local.info.kms_keys[var.env], null) != null ? [1] : []
    content {
      default_kms_key_name = local.info.kms_keys[var.env]
    }
  }
}
...
module "cloud_function" {
  ...
  kms_key_name          = local.info.kms_keys[var.env]
